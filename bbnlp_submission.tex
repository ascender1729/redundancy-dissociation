...
frozen, reviewer-reproducible estimator, the surviving results are (i) a weak but consistent
\emph{negative} association between redundancy and decoder-geometry universality across three model pairs
($N{=}2500$ features per pair; partial $\rho\in[-0.17,-0.12]$, all outside a permutation null), which on
one pair strengthens beyond the shuffle null ($-0.23$), and (ii) a co-firing association that is real but
attributable to context breadth. We release per-feature arrays and a recompute script so every number
...
dangerously sensitive to the ridge floor and the partialling convention.
\end{abstract}
...
incremental partial of redundancy given the null is indistinguishable from zero for co-firing on all
pairs ($-0.09$ to $+0.02$) while the reverse partial retains signal ($+0.12$ to $+0.26$). The
shuffle-\emph{invariant} component carries the co-firing association; the shuffle-\emph{sensitive}
(position-structure) component adds nothing detectable. For decoder geometry the picture differs by pair:
on the Gemma pair the negative association \emph{strengthens} beyond the null (incremental $-0.23$),
while on the other two pairs it, too, is largely carried by the shuffle-invariant component.
...
observed partial $\rho$; ``null'' uses the position-shuffled redundancy score; the last two columns are
incremental partials of each predictor given the other. The co-firing headline never beats the null and
retains no unique signal beyond it.}
\label{tab:null}
\end{table}
...
$U_{\text{overlap}}$ cells lie outside their per-cell permutation bands (widest $[-0.043,+0.040]$;
$p=0.001$); the Gemma
...
coefficient outside the permutation null (Table~\ref{tab:main}, Figure~\ref{fig:forest}). The measure is
alive (it beats its shuffled-decoder baseline in every run), so this is a real weak negative, not a dead
measure. Against the position-shuffle null the negative is largely carried by the context-breadth
...
on at least 2 of 3 pairs; only the near pair clears $+0.20$, and the Mistral pair falls below it, so C1
is \emph{confirmed on only 1 of 3 pairs}. More decisively, Section~\ref{sec:null} shows the association
...
representation geometry, and treat breadth-of-firing as the boring-but-live confounder behind
